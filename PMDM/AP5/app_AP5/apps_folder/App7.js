    ...
    setArticles((prevArticles) =>
      prevArticles.map((article) =>
        article.id === id
          ? {
              ...article,
              isToggled: !article.isToggled,
              title: article.isToggled ? `Título ${id}` : `Nuevo título para el Artículo ${id}`,
              imageUrl: article.isToggled
                ? "https://via.placeholder.com/150"
                : `https://via.placeholder.com/150?text=Imagen+${id}`,
              body: article.isToggled
                ? `Este es el contenido del artículo ${id}.`
                : `Este es el nuevo contenido del Artículo ${id}.`,
            }
          : article
      )
    );
